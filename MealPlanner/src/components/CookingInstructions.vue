...
<template>
	...
	<v-container v-else fluid>
		<v-col>
			<v-img v-if="recipe.image" :src="recipe.image" cover height="200" />
			<h1 class="text-h3 ma-4">{{ recipe.title }}</h1>

			<recipe-rating :id="recipe.id" />

			<v-chip v-for="cuisine in recipe.cuisines" :key="cuisine" class="ma-2 my-4" color="primary">
				{{ cuisine }}
			</v-chip>
			...
			<v-expansion-panels v-model="panel" variant="accordion">
				<v-expansion-panel>
					...
					<v-expansion-panel-text>
						<div class="text-body-1" v-html="recipe.summary"></div>
					</v-expansion-panel-text>
				</v-expansion-panel>
				<v-expansion-panel>
					<v-expansion-panel-title class="text-h5">Instructions</v-expansion-panel-title>
					<v-expansion-panel-text>
						<div class="text-body-1" v-html="recipe.instructions"></div>
					</v-expansion-panel-text>
				</v-expansion-panel>
			</v-expansion-panels>
		</v-col>
