...
<template>
	<div>
		<v-card>
			...
			<v-list>
				<v-list-item v-for="(answer, index) in answersList" :key="`answer-${index}`" class="my-2">
					<v-list-item-content>
					...
					<template v-slot:append>
						<v-icon v-if="isAnswerCorrect(index)" color="success" icon="mdi-check-bold"></v-icon>
						<v-icon
