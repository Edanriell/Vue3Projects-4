			...
			<v-list>
				<v-list-item v-for="(item, index) in detected" :key="index">
					...
					<v-list-item-subtitle
						>Confidence {{ roundNumber(item.score * 100, 2) }}%</v-list-item-subtitle
					>
				</v-list-item>
			</v-list>
			<TextToSpeech v-if="speech" :message="speech"></TextToSpeech>
		</div>
	</v-container>
</template>
...
<script lang="ts" setup>
	...
	const uniqueObjects = computed(() => {
		return detected.value.filter((v, i, a) => a.indexOf(v) === i).map((v) => v.class);
	});
